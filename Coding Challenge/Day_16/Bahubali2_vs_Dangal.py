# -*- coding: utf-8 -*-
"""
Created on Mon May 27 12:02:09 2019
...
label1 = dataset.iloc[:, 1].values.reshape(9,1)
label2 = dataset.iloc[:, 2].values.reshape(9,1)
...
if regressor1.predict(x) > regressor2.predict(x):
    print("Bahubali has more collections:",regressor1.predict(x))
else:
    print("Dangal has more collections:",regressor2.predict(x))
    """
import pandas as pd
import matplotlib.pyplot as plt
...
x1 = dataset.iloc[:, 0:1].values.reshape(9,1)
y1 = dataset.iloc[:, 1:2].values.reshape(9,1)  # Bahubali 2
y2 = dataset.iloc[:, 2:3].values.reshape(9,1)   # Dangal
